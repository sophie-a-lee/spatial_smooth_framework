...
## NOTE: this code will only work if 01_simstudy2_simulation.R and
## 02_simstudy2_run_models.R has been run
df_3phi_sim <- read_rds("data/df_sim_simstudy2.rds")

model_list <- read_rds("output/sim_study2/model_list.rds")
...
# Save intercept plot
ggsave(b_line, filename = "output/sim_study2/intercept_line.png")
...
phi_est <- reduce(model_vars, rbind) %>% 
  #  Add true phi value
  mutate(phi_human = rep(seq(0, .9, by = .1), 
...
phi_dist_line <- ggplot(data = phi_est) +
  ...
  coord_cartesian(xlim = c(-.05, .95), y = c(-.01, 1), expand = F) +
...
ggsave(phi_dist_line, 
       filename = "output/sim_study2/phi_line_dist.png")
...
ggsave(phi_human_line, 
       filename = "output/sim_study2/phi_line_human.png")
...
ggsave(phi_iid_line, 
       filename = "output/sim_study2/phi_line_iid.png")
